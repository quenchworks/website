...
FROM ghcr.io/quenchworks/images/yarn:1.22.22 AS build
USER root
WORKDIR /app
...
FROM ghcr.io/quenchworks/images/yarn:1.22.22 AS prod-deps
USER root
WORKDIR /app
ENV YARN_CACHE_FOLDER=/tmp/yarn
COPY package.json yarn.lock ./
...
FROM ghcr.io/quenchworks/images/node:26.10.0 AS runtime
WORKDIR /app

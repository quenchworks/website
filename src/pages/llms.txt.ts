...
import { images, availableCount } from '../data/images';
...
export const GET: APIRoute = () => {
  ...
  const imageList = images.filter((i) => i.status === 'available');
  ...
  lines.push(
    `> Free, independent catalog of hardened, 0-CVE container images and signed Helm charts. ` +
      ...
      `replacement for the Bitnami catalog. ${availableCount} hardened images and ${chartCount} signed charts.`,
  );
  lines.push('');
  lines.push(
  ...
  lines.push(`- [All images](${SITE}/images): ${availableCount} hardened, 0-CVE container images`);
  lines.push('');

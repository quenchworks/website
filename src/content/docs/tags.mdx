...
There is no `:latest`. A bare reference like `docker pull ghcr.io/quenchworks/images/redis` resolves to `:latest` and fails with `MANIFEST_UNKNOWN`. Always pull a version tag. The current version is shown on every [image](/images) page, or you can list the tags yourself:
